...
def get_gemini_response(input, text):
    ...
    If you are unable to generate an answer, prompt the user to ask a question related to the provided text.
    """
...
async def predict(question: str = Form(...), file: UploadFile = File(None)):
    ...
    if not question:
        raise HTTPException(status_code=400, detail="Please type something.")
